...
WEB_APP_URL = "https://script.google.com/macros/s/AKfycbxwYFu95Y_v5QtLzXgODHEFrPNdujkh21zWFbxZ1crQfdaYo1U37VbVqwfFWCLwTRQr/exec"
...
# ==========================================================
# 1. TRANG NHẬP BÁO CÁO (DÀNH CHO CƠ SỞ)
# ==========================================================
if menu == "📝 Nhập Báo Cáo (Cơ sở)":
    ...
    if not st.session_state["dang_nhap_co_so"]:
        ...
        with st.form("form_bao_cao"):
            # TOÀN BỘ BIỂU MẪU NHẬP LIỆU HÔM QUA GIỮ NGUYÊN
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    if len(data_json) > 0:
                        ...
                        tong_hoi_nghi = pd.to_numeric(df.iloc[:, 4], errors='coerce').sum() # Cột 4 là Số hội nghị
                        tong_mo_hinh = pd.to_numeric(df.iloc[:, 7], errors='coerce').sum() # Cột 7 là Mô hình DV
                        
                        m1, m2, m3 = st.columns(3)
                        with m1:
                            st.markdown(f'<div class="metric-box"><div class="metric-label">🏢 Số Báo cáo đã nộp</div><div class="metric-value">{tong_don_vi_da_nop} / 124</div></div>', unsafe_allow_html=True)
                        with m2:
                        ...
                        with col_chart1:
                            ...
                            df_ky_so = df.iloc[:, [1, 9]] # Cột 1 là Đơn vị, Cột 9 là Ký số
                        ...
                        with col_chart2:
                            ...
                            df_tin_bai = df.iloc[:, [1, 6]] # Cột 6 là Tin bài
                        ...
                        st.dataframe(df.iloc[:, [0, 1, 2, 3]]) # Chỉ hiện Thời gian, Đơn vị, Kỳ báo cáo, Người lập
                    else:
