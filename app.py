...
# --- MẬT KHẨU BẢO MẬT ---
MAT_KHAU_HE_THONG = "TGDV@2026"  # <--- BẠN CÓ THỂ ĐỔI MẬT KHẨU Ở ĐÂY

# Khởi tạo bộ nhớ tạm để nhớ trạng thái đăng nhập
...
if not st.session_state["dang_nhap_thanh_cong"]:
    ...
    if st.button("🔑 Mở khóa", use_container_width=True):
        if mat_khau_nhap == MAT_KHAU_HE_THONG:
            ...
            st.rerun() # Tải lại trang để hiện biểu mẫu
        else:
            st.error("❌ Mật khẩu không chính xác! Đồng chí vui lòng kiểm tra lại.")
            
else:
    # MÀN HÌNH CHÍNH (Sau khi đã mở khóa)
        ...
        if submitted:
            if don_vi == "Chọn đơn vị...":
                ...
                with st.spinner("⏳ Đang mã hóa file và gửi dữ liệu lên máy chủ Tỉnh ủy..."):
                    ...
                    WEB_APP_URL = "https://script.google.com/macros/s/AKfycbyebYTTM2LwI3EFG3SNg6HIoDf-rtNiY0ZJNJ69H3LcIDr86NBWeUXv_w00ElJ90xfa/exec"
