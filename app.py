...
<style>
    ...
    .sub-title { font-size: 16px; font-weight: bold; color: #004B87; text-align: center; text-margin-bottom: 20px;}
...
# --- BIỂU MẪU NHẬP LIỆU ---
# Tạo Form để gom toàn bộ dữ liệu, chỉ khi bấm "Gửi báo cáo" mới load trang
with st.form("form_bao_cao"):
    
    st.markdown('<div class="section-header">📍 PHẦN 1: THÔNG TIN ĐƠN VỊ</div>', unsafe_allow_html=True)
    # Danh sách cấp xã/phường trực thuộc tỉnh (Đã cập nhật theo hệ thống chính quyền 2 cấp mới)
    danh_sach_don_vi = [
        "Chọn đơn vị...", 
        "Phường Minh Xuân", "Phường Tân Quang", "Xã Lưỡng Vượng", # Khu vực Tuyên Quang cũ
        "Phường Minh Khai", "Phường Nguyễn Trãi", "Xã Phương Độ", "Xã Thanh Thủy", # Khu vực Hà Giang sáp nhập
        "Xã Bắc Mê", "Thị trấn Yên Phú", "Xã Yên Lập" # Các đơn vị khác...
    ]
    ...
    zalo_oa = st.radio("2. Tiến độ triển khai Zalo / Mini App:", 
                       ["Đã vận hành thường xuyên", "Đang thử nghiệm", "Chưa triển khai"])
    c5, c6 = st.columns(2)
    with c5:
    ...
    if submitted:
        if don_vi == "Chọn đơn vị...":
            ...
            st.success(f"✅ Đã gửi thành công báo cáo {ky_bao_cao} của {don_vi}! Hệ thống đang đồng bộ dữ liệu...")
            st.balloons()
